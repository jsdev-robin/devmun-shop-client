...
import { getPinStyles } from '../../common/getPinStyles';
import RowIsLoadingSkeleton from '../../common/RowIsLoadingSkeleton';

const GridLoadingSkeleton = () => {
  const { table, split } = useDataLayoutContext();
      ...
      {[...Array(20)].map((_, i) => (
        <Tr key={i} className="*:border-r *:border-border h-10">
          {visibleColumns.map((column, j) => (
            ...
            >
              <RowIsLoadingSkeleton column={column} i={i} j={j} />
            </Td>
          ))}
        </Tr>
      ))}
